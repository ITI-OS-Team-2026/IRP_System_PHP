...
<div class="min-h-screen flex">
    ...
    <main class="flex-1 p-8">
        <header class="mb-6">
            <h1 class="text-2xl font-bold text-indigo-900">زمام الموافقات النهائية</h1>
            <p class="text-slate-600 mt-1">الأبحاث الموافق عليها من المراجع ولم تُصدر لها شهادة بعد.</p>
        </header>
            ...
            <table class="w-full text-right">
                <thead class="bg-slate-100 border-b border-slate-200">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-sm">تاريخ المراجعة</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    <?php if (empty($submissions)): ?>
                        <tr>
                            <td colspan="5" class="px-4 py-12 text-center text-slate-500">لا توجد طلبات بانتظار القرار النهائي.</td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($submissions as $item): ?>
                            <tr class="hover:bg-slate-50">
                                ...
                                <td class="px-4 py-3 text-sm">
                                    <a href="<?php echo BASE_URL; ?>/committee/certificate/<?= (int) $item['id'] ?>" class="inline-flex items-center gap-2 rounded-md bg-indigo-900 px-4 py-2 text-white font-bold hover:bg-indigo-800 transition-colors">
